...
Tujuan dari frontend ini adalah memberikan pengalaman pengguna (UX) selayaknya aplikasi Web2 konvensional (Web2.5), di mana kerumitan blockchain disembunyikan semaksimal mungkin. 
...
2. **Keterbatasan Performa RPC**: Diselesaikan menggunakan lapisan sinkronisasi *database* pragmatis (Supabase) alih-alih melakukan pemanggilan RPC *looping*.
...
│   │   ├── page.tsx                # Landing Page (Daftar Event/Tiket via Supabase)
...
## 4.5 Arsitektur Pengambilan Data (Mengatasi Celah Performa RPC)

**Masalah:** 
*Smart contract* menyimpan data tiket dalam struktur `mapping(uint256 => Listing)`. Melakukan *looping* pemanggilan RPC (contoh: `getListing(1)`, `getListing(2)`) langsung dari Next.js untuk merender ribuan daftar tiket akan menghabiskan kuota API RPC (seperti Alchemy/Infura) dalam sekejap dan membuat *website* sangat lambat (*lagging*).

**Solusi Pragmatis:** 
Menggunakan *database* lokal (seperti **Supabase/PostgreSQL**) sebagai lapisan indeks.
- **Sinkronisasi (Backend):** Sebuah skrip/layanan berjalan di *background* untuk mendengarkan (*listen*) *event* on-chain seperti `TicketListed` dan `TicketSold`, lalu mencatatnya ke *database* Supabase.
- **Pembacaan (Frontend):** Halaman *Landing Page* dan *Marketplace* mengambil daftar tiket langsung dari *database* via API REST standar. Hasilnya instan (dalam hitungan milidetik) tanpa membebani RPC.
- **Penulisan (Smart Contract):** Saat pengguna menekan tombol "Beli", eksekusi mutlak tetap dilempar ke dompet pengguna dan diselesaikan di *smart contract* untuk validasi keamanan (ketersediaan tiket, *price ceiling*, waktu mulai).

*(Catatan: Untuk ekosistem Web3 berskala besar, praktik idealnya adalah menggunakan perangkat khusus Indexer seperti The Graph atau Ponder, namun Supabase adalah opsi yang jauh lebih cepat untuk target waktu penyelesaian skripsi).*
...
export function useBuyTicket() {
  ...
  const [pendingPurchase, setPendingPurchase] = useState<{listingId: bigint, amount: number} | null>(null)

  const executePurchase = async (listingId: bigint, amount: number, totalPrice: string) => {
    try {
      setTxState('approving')
      setPendingPurchase({ listingId, amount })
  ...
  useEffect(() => {
    if (isApproveSuccess && txState === 'approving' && pendingPurchase) {
      ...
      writeBuy({
        ...
        args: [pendingPurchase.listingId, BigInt(pendingPurchase.amount)],
      })
...
## 4.8 Panduan Menjalankan Frontend Lokal
